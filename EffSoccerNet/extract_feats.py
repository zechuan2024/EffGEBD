...
if __name__ == '__main__':
    ...
    root = '/data1/SoccerNet/videos/'
    video_dirs = getListGames('train', task="camera-changes") + getListGames('test', task="camera-changes")
        ...
        for half in ['1', '2']:
            ...
            for t in range(0, total_t, max_len):
                ...
                with torch.no_grad():
                    ...
                    x = model.layer1(x)
                    x = model.layer2(x)
                    features_l2.append(x.mean([-2, -1]).cpu())
                    x = model.layer3(x)
                    x = model.layer4(x)
                    features_l4.append(x.mean([-2, -1]).cpu())
